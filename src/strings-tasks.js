...
function removeLastOccurrences(str, value) {
  const startPos = str.lastIndexOf(value);
  return startPos === -1
    ? str
    : `${str.slice(0, startPos)}${str.slice(startPos + value.length)}`;
}
...
function countVowels(str) {
  return str.split('').filter((item) => item.match(/[AEIOUY]/i)).length;
}
...
function isPalindrome(str) {
  const checkStr = str.toLocaleLowerCase().replaceAll(/[^a-z0-9]/g, '');
...
function invertCase(str) {
  return str
    .split('')
    .map((item) =>
      item.match(/[A-Z]/) ? item.toLowerCase() : item.toUpperCase()
    )
    .join('');
}
